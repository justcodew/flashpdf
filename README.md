...
> - 文本提取 mean **6.25ms** / p50 **1.19ms**（165/165 零失败，0% 失败率）
> - 比 pdf_oxide 快 **2.9×**、比 liteparse 快 **2.8×**（按总耗时）
...
`pip install` 的 wheel **自带 PDFium binary**（~7MB），渲染功能（`page.get_pixmap()`）开箱即用，无需额外配置。
...
- **页面渲染**（`pip install flashpdf` 自带 PDFium binary）：`page.get_pixmap(dpi=150)` 返回 PNG bytes
...
| 渲染部署便利（pip 装即用）| ✅ wheel 自带 PDFium | pypdfium2（同样自带 PDFium）|
...
**全文提取总耗时（165 文件累加）**：flashpdf **1.03s** vs pdf_oxide 2.91s vs liteparse 2.99s。

**速度倍数（corpus 总耗时）**：vs pdf_oxide **2.83×**、vs liteparse **2.90×**。

**单文件速度比（geo-mean，other / flashpdf）**：vs pdf_oxide **1.47×**、vs liteparse **1.56×**。

**按文件大小分桶（p50 ms）**：

| 桶 | n | flashpdf | pdf_oxide | liteparse |
|---|---:|---:|---:|---:|
...
**结论**：flashpdf 在 large 桶优势最大（**2.3-2.7×**），medium 桶 2× 左右；tiny/small 桶
三库接近（差距在 0.5ms 量级，绝对值很小）。RAG 索引、批量预处理、大文档解析等"重负载"
场景 flashpdf 是首选；发票/邮件附件等小文件批量场景优势最小但不落下风。

单文件重负载场景（14-15 页 arxiv 论文 + rayon 多核加速）下 flashpdf 还可达更高领先倍数，
...
```bash
cargo test -p flashpdf-core    # 161 个核心单元测试
